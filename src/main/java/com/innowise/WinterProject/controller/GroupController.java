...
public class GroupController {
    ...
    public ResponseEntity<?> removeGroup(@PathVariable UUID id) {
        groupService.removeGroup(id);
        return ResponseEntity.ok().build();
    }

    @PutMapping
    public ResponseEntity<GroupDto> updateStudent(@RequestBody @Validated(Update.class) GroupDto groupDto) {
        return ResponseEntity.ok(groupMapper.groupToDto(
